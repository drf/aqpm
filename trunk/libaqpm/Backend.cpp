...
void Backend::updateKdemodDatabase()
{
    d->upThread = new UpDbThread(d->dbs_sync, this);

    connect(d->upThread, SIGNAL(finished()), SIGNAL(dbUpdated()));
    connect(d->upThread, SIGNAL(error(const QString&)), SIGNAL(errorOccurred(const QString&)));

    d->upThread->start();
}

QStringList Backend::getInstalledKdemodPackages()
{
    QStringList retlist;

    alpm_list_t *list = alpm_db_getpkgcache(d->db_local);

    while (list != NULL) {
        QString name = QString(alpm_pkg_get_name((pmpkg_t *)alpm_list_getdata(list)));

        if (name.startsWith("kdemod")) {
            retlist.append(name);
        }
...
QStringList Backend::getAvailableKdemodPackages()
{
    QStringList retlist;
    alpm_list_t *dbs = alpm_option_get_syncdbs();

    while (dbs != NULL) {
        if (!QString(alpm_db_get_name((pmdb_t *)alpm_list_getdata(dbs))).startsWith("kdemod")) {
            dbs = alpm_list_next(dbs);
            continue;
        }

        alpm_list_t *list = alpm_db_getpkgcache((pmdb_t *)alpm_list_getdata(dbs));

        while (list != NULL) {
            retlist.append(QString(alpm_pkg_get_name((pmpkg_t *)alpm_list_getdata(list))));
...
void Backend::installKdemod(InstallMode mode, const QStringList &packages)
{
    QStringList tmp;
    QStringList toSync;
    alpm_list_t *tmplist;
    alpm_list_t *tmplistb;

    switch (mode) {
    case KeepPackages:
        tmp = getAvailableKdemodPackages();

        foreach(const QString &package, getInstalledKdemodPackages()) {
            if (tmp.contains(package)) {
                toSync.append(package);
            }
        }
        break;
    case InstallComplete:
        tmplist = alpm_db_getgrpcache(d->dbs_sync);

        while (tmplist != NULL) {

            if (QString(alpm_grp_get_name((pmgrp_t *)alpm_list_getdata(tmplist))) == "kdemod-complete") {

                tmplistb = alpm_grp_get_pkgs((pmgrp_t *)alpm_list_getdata(tmplist));

                while (tmplistb != NULL) {
                    toSync.append(QString(alpm_pkg_get_name((pmpkg_t *)alpm_list_getdata(tmplistb))));

                    tmplistb = alpm_list_next(tmplistb);
                }

                break;
            }

            tmplist = alpm_list_next(tmplist);
        }
        break;
    ...
    case InstallBase:
        ...
        while (tmplist != NULL) {
            ...
            if (QString(alpm_grp_get_name((pmgrp_t *)alpm_list_getdata(tmplist))) == "kdemod-base") {
    ...
    d->trThread = new TrCommitThread(PM_TRANS_TYPE_SYNC, toSync, this);

    connect(d->trThread, SIGNAL(finished()), SLOT(computeTransactionResult()));
    connect(d->trThread, SIGNAL(error(const QString&)), SIGNAL(errorOccurred(const QString&)));

    d->trThread->start();
}

void Backend::removeAllKdemodPackages()
{
    d->trThread = new TrCommitThread(PM_TRANS_TYPE_REMOVE, getInstalledKdemodPackages(), this);
